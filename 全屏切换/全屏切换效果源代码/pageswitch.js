(function($){
    ...
    //用于得到window的jquery对象，和定义容器和子容器jquery对象
	var win = $(window),
		container,sections;   
	//定义全局的对象，和是否滑动
	var opts = {},
	...
	var SP = $.fn.switchPage = function(options){
        ...
        //把图片的dom对象装换为jquery对象
		sections.each(function(){
			arrElement.push($(this));
		});
		
		//return是为了能够继续jquery的连写，其实就是返回了一个jquery对象，不过改变了其中的一些属性，元素
	...
	function scrollPage(element){
		var dest = element.position();  //获得现在的位置
	...
	function MouseWheelHandler(e) {
		e.preventDefault();
		//在jquery中event.originalEvent  指向原始对象，及指向浏览器的原本对象
		//火狐和ie的鼠标滑轮滚动事件，大于0向上，小于0乡下，火狐时大于零的向下，小于0的向上
		var value = e.originalEvent.wheelDelta || -e.originalEvent.detail;  //此处是吧火狐和ie的浏览器的滑轮滚动参数对其
		var delta = Math.max(-1, Math.min(1, value));  //此保证只有1和-1的产生
		if(canScroll){
			if (delta < 0) {
				SP.moveSectionDown();
			}else {
	...
	function initLayout(){
		var length = sections.length,
			width = (length*100)+"%",
			cellWidth = (100/length).toFixed(2)+"%";  //表示最多有俩位小数
